...
wxDEFINE_EVENT(EVT_GCODE_VIEWER_CHANGED, SimpleEvent);
...
bool Sidebar::priv::sync_extruder_list(bool &only_external_material, bool force_printer_select)
{
    ...
    if (obj == nullptr || !obj->is_online() || force_printer_select) {
        // Instead of redirecting to Device tab, show printer selection dialog
        ...
        if (available_printers.empty()) {
            // No printers available, show original error
            plater->pop_warning_and_go_to_device_page(printer_name, Plater::PrinterWarningType::NOT_CONNECTED, _L("Sync printer information"));
            return false;
        }
...
public:
    ...
    void on_gcode_viewer_changed(SimpleEvent&);
...
void Plater::priv::on_gcode_viewer_changed(SimpleEvent&)
{
    if (!this->preview) {
        return;
    }
    this->preview->reload_print(false, q->m_only_gcode);
}
...
bool Plater::try_sync_preset_with_connected_printer(int& nozzle_diameter)
{
    ...
    MachineObject* obj = dev->get_selected_machine();
    if (!obj || !obj->is_info_ready() || obj->GetExtderSystem()->GetTotalExtderCount() <= 0) {
...
void Plater::calib_flowrate(bool is_linear, int pass)
{
    ...
    // Determine calibration name based on mode
    wxString calib_name;
    if (is_linear) {
        calib_name = L"YOLO Flow Calibration";
        if (pass == 2)
    ...
    // Load appropriate 3MF model based on mode
    if (is_linear) {
        if (pass == 1)
            add_model(false, (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "filament_flow" / "Orca-LinearFlow.3mf").string());
        else
            add_model(false, (boost::filesystem::path(Slic3r::resources_dir()) / "calib" / "filament_flow" / "Orca-LinearFlow_fine.3mf").string());
    } else {
        if (pass == 1)
    ...
    // scale z to have appropriate layers (10 for YOLO, 7 for standard)
    ...
    // Get flow ratio for YOLO mode
    double cur_flowrate = 1.0;
    if (is_linear) {
        auto flow_ratio_opt = filament_config->option<ConfigOptionFloats>("filament_flow_ratio");
        if (flow_ratio_opt && flow_ratio_opt->values.size() > 0)
            cur_flowrate = flow_ratio_opt->get_at(0);
    }

    std::vector<double> internal_solid_speeds = generate_max_speed_parameter_value("internal_solid_infill_speed");
    std::vector<double> top_surface_speeds = generate_max_speed_parameter_value("top_surface_speed");

    // adjust parameters
    for (auto _obj : model().objects) {
        _obj->ensure_on_bed();
        // YOLO: 1 wall, Standard: 3 walls
        _obj->config.set_key_value("wall_loops", new ConfigOptionInt(is_linear ? 1 : 3));
        ...
        // YOLO: 2 bottom layers, Standard: 1
        _obj->config.set_key_value("bottom_shell_layers", new ConfigOptionInt(is_linear ? 2 : 1));
        ...
        // YOLO: Archimedean Chords, Standard: Monotonic
        _obj->config.set_key_value("top_surface_pattern", new ConfigOptionEnum<InfillPattern>(is_linear ? ipArchimedeanChords : ipMonotonic));
        ...
        // Safely parse modifier from object name
        if (obj_name.length() > 9) {
            std::string modifier_str = obj_name.substr(9);
            if (!modifier_str.empty() && modifier_str[0] == 'm')
                modifier_str[0] = '-';

            // Force set locale to C to avoid parsing error
            const std::string _loc = std::setlocale(LC_NUMERIC, nullptr);
            std::setlocale(LC_NUMERIC, "C");
            try {
                modifier = std::stof(modifier_str);
            } catch (...) {
                modifier = 0.0f;
            }
            // restore locale
        ...
        if (is_linear) {
            // YOLO mode: modifier is direct flow offset (e.g., -0.02, -0.01, 0, 0.01, 0.02)
            if (cur_flowrate > 0)
            ...
            // Standard mode: modifier is percentage (e.g., -10, -5, 0, 5, 10)
    ...
    try {
        json js;
        if (is_linear) {
            js["cali_type"] = pass == 1 ? "yolo_flow_rate_1" : "yolo_flow_rate_2";
        } else {
            js["cali_type"] = pass == 1 ? "third_cali_flow_rate_1" : "third_cali_flow_rate_2";
        }
...
void Plater::calib_temp(const Calib_Params &params)
{
    ...
    set_config_values<int, ConfigOptionIntsNullable>(filament_config, "nozzle_temperature", (int) start_temp);
...
int Plater::export_3mf(const boost::filesystem::path& output_path, SaveStrategy strategy, int export_plate_idx, Export3mfProgressFn proFn)
{
    ...
    BOOST_LOG_TRIVIAL(trace) << __FUNCTION__ << boost::format(": path=%1%, backup=%2%, export_plate_idx=%3%, SaveStrategy=%4%") % PathSanitizer::sanitize(output_path) %
