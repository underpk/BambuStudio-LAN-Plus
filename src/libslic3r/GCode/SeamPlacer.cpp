                                      ...
                                      SeamPosition                            seam_position = spAligned)
{
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, result.size()), [&triangles, &precomputed_sample_directions, model_contains_negative_parts, negative_volumes_start_index,
        ...
        for (size_t s_idx = r.begin(); s_idx < r.end(); ++s_idx) {
            ...
            // For Aligned Back: penalize front-facing surfaces to prioritize seams away from front
                              ...
                              SeamPosition seam_position = spAligned)
{
    ...
    result.mesh_samples_visibility = raycast_visibility(raycasting_tree, triangle_set, result.mesh_samples, negative_volumes_start_index, seam_position);
    throw_if_canceled();
#ifdef DEBUG_FILES
...
struct SeamComparator
{
    ...
    {
        if ((setup == SeamPosition::spAligned || setup == SeamPosition::spAlignedBack) && a.central_enforcer != b.central_enforcer) { return a.central_enforcer; }
...
void SeamPlacer::init(const Print &print, std::function<void(void)> throw_if_canceled_func)
{
    ...
    for (const PrintObject *po : print.objects()) {
...

        {
            ...
            throw_if_canceled_func();
            if (configured_seam_preference == spAligned || configured_seam_preference == spAlignedBack || configured_seam_preference == spNearest) { compute_global_occlusion(global_model_info, po, throw_if_canceled_func, configured_seam_preference); }
            throw_if_canceled_func();
            ...
            throw_if_canceled_func();
            if (configured_seam_preference == spAligned || configured_seam_preference == spAlignedBack || configured_seam_preference == spNearest) {
        ...
        throw_if_canceled_func();
        if (configured_seam_preference == spAligned || configured_seam_preference == spAlignedBack || configured_seam_preference == spRear) {
...
void SeamPlacer::place_seam(const Layer *layer, ExtrusionLoop &loop, bool external_first, const Point &last_pos, bool &satisfy_angle_threshold) const
{
    ...
    if (const SeamCandidate &perimeter_point = layer_perimeters.points[seam_index];
        (po->config().seam_position == spNearest || po->config().seam_position == spAligned || po->config().seam_position == spAlignedBack) && loop.role() == ExtrusionRole::erPerimeter && // Hopefully internal perimeter
