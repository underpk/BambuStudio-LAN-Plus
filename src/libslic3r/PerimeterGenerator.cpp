...
void PerimeterGenerator::process_classic()
{
    ...
    // Orca: Bridge counterbore holes - create a copy and process it
            ...
            if (!unsupported.empty()) {
                ...
                if (!unsupported_filtered.empty()) {
                    ...
                    if (!unsupported.empty()) {
                        ...
                        for (ExPolygon unsupp : unsupported_filtered) {
                            BridgeDetector detector{ unsupp,
                                                    lower_island.expolygons,
                                                    perimeter_spacing };
                            if (detector.detect_angle(Geometry::deg2rad(this->config->bridge_angle.value)))
                                append(bridgeable, union_ex(detector.coverage(-1)));
                        }
                        if (!bridgeable.empty()) {
